...
// Mapeo de indicadores para la estación "CodeGo"
const indicatorsMeta = [
    { key: "logicaSecuencial", label: "Lógica secuencial", desc: "Programa pasos en orden correcto para que el robot complete la ruta." },
    { key: "pensamientoComputacional", label: "Pensamiento computacional", desc: "Descompone la tarea en pasos simples y los relaciona entre sí." },
    { key: "planificacionEstrategica", label: "Planificación estratégica", desc: "Anticipa obstáculos y diseña rutas alternativas si es necesario." },
    { key: "visionEspacial", label: "Visión espacial", desc: "Comprende direcciones y giros en el espacio donde se mueve el robot." },
    { key: "coordinacionGeneral", label: "Coordinación general", desc: "Mantiene control físico al organizar tarjetas/pistas sin errores en la ejecución." },
    { key: "trabajoBajoPresion", label: "Trabajo bajo presión", desc: "Realiza la actividad dentro de un tiempo límite sin bloquearse emocionalmente." },
    { key: "autorregulacionEmocional", label: "Autorregulación emocional", desc: "Se calma y retoma la tarea cuando comete errores o el robot no sigue la ruta prevista." },
];
...
// ===============================================
export default function DashDoctorCodeGo() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            // SIMULACIÓN DE DATOS (Mismos que el ejemplo anterior)
            const patientData = [{
     ...
     * @param {string} key - La clave del indicador.
    ...
    async function handleSubmit(e) {
        ...
        // 1. Consolidar Observaciones (para posible API de guardado)
        ...
        // 2. Mapeo de indicadores de CodeGo al Payload de la API ML (DiagnosticMLMechanicalArm)
        // **IMPORTANTE**: Este mapeo es una suposición, debe coincidir con la lógica del backend
        const mlData = {
            gender: patient.patientProfileFreeGender === "Femenino" ? "F" : "M", 
            patientAge: age,
            // Las claves se mapean a los campos que la API ML espera:
            sustainedAttention: evaluationValues.logicaSecuencial.score,       // Lógica Secuencial
            planning: evaluationValues.planificacionEstrategica.score,          // Planificación Estratégica
            categorization: evaluationValues.pensamientoComputacional.score,     // Pensamiento Computacional
            executiveFunction: evaluationValues.coordinacionGeneral.score,      // Coordinación General
            eyeHandCoordination: evaluationValues.visionEspacial.score,         // Visión Espacial
            fineMotorPrecision: evaluationValues.trabajoBajoPresion.score,      // Trabajo Bajo Presión
            toleranceFrustration: evaluationValues.autorregulacionEmocional.score, // Autorregulación Emocional
            // NOTA: El modelo anterior usaba 8 indicadores. Si solo tienes 7 aquí, 
            // asegúrate que el backend maneje el índice 8 (perseverance) o ajusta el mapeo.
            perseverance: evaluationValues.autorregulacionEmocional.score, // Duplicamos para mantener 8 campos o ajusta si falta uno.
        };
